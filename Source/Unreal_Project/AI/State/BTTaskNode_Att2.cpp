...
#include "AI/State/BTTaskNode_Att2.h"
...
EBTNodeResult::Type UBTTaskNode_Att2::ExecuteTask(UBehaviorTreeComponent& _OwnerComp, uint8* NodeMemory)
{
	Super::ExecuteTask(_OwnerComp, NodeMemory);

	EMonsterState State = GetCurState<EMonsterState>(_OwnerComp);

	if (State != EMonsterState::Attack2)
	{
...
void UBTTaskNode_Att2::TickTask(UBehaviorTreeComponent& _OwnerComp, uint8* _pNodeMemory, float _DeltaSeconds)
{
