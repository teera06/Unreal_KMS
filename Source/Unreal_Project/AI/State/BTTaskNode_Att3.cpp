...
EBTNodeResult::Type UBTTaskNode_Att3::ExecuteTask(UBehaviorTreeComponent& _OwnerComp, uint8* NodeMemory)
{
	Super::ExecuteTask(_OwnerComp, NodeMemory);

	EMonsterState State = GetCurState<EMonsterState>(_OwnerComp);

	if (State != EMonsterState::Attack3)
	{
...
void UBTTaskNode_Att3::TickTask(UBehaviorTreeComponent& _OwnerComp, uint8* _pNodeMemory, float _DeltaSeconds)
{
